...
module.exports = class GraphService {
  constructor () {
    ...
    console.log(`🚀 Conectado a Neo4j en ${process.env.NEO4J_DATABASE_URL}`)
  }

  createNode = async (entity, data) => {
    try {
      const entityCamelCase = entity.charAt(0).toLowerCase() + entity.slice(1)
      console.log(`🔘 Creando nodo ${entity} con datos:`, data)
  ...
  createRelation = async (entity, relation, relatedEntity, data) => {
    try {
      ...
      console.log(`🔘 Creando relación ${relation} entre ${entity} y ${relatedEntity}`)
  ...
  getRelated = async ({ entity, entityId, relation, entityConnected, where, limit = 10 }) => {
    console.log(`🔘 Obteniendo relaciones ${relation} entre ${entity} y ${entityConnected}`)
    ...
    try {
      ...
      console.log('✅ Relaciones obtenidas:', records)
  ...
  getRelatedAndSelf = async ({ entity, entityId, relation, entityConnected, where, limit = 10 }) => {
    ...
    try {
      ...
      console.log('✅ Relaciones obtenidas:', result)
  ...
  async close () {
    console.log('👋 Cerrando conexión a Neo4j')
