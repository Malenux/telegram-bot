class Activation extends HTMLElement {
  ...
  render () {
    this.shadow.innerHTML = /* html */`
      ...
      <div class="content">
        <div class="text">
          ...
          <h2>Casi listo</h2>
          <p>Crea una contraseña segura para activar tu cuenta.</p>
        </div>
    ...
    this.shadow.querySelector('form').addEventListener('submit', async (event) => {
      ...
      if (!password || !repeatPassword) {
        messageElement.textContent = 'Los campos no pueden estar vacíos'
        return
      }

      if (!regex.test(password)) {
        messageElement.textContent = 'La contraseña no cumple con los requisitos mínimos'
        return
      }
      ...
      if (response.ok) {
        messageElement.textContent = 'Cuenta activada correctamente'
