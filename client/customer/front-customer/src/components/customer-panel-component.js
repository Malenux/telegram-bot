class CustomerPanel extends HTMLElement {
  constructor () {
    super()
  ...
  async loadCustomerData () {
    try {
      ...
      if (result.ok) {
        this.data.customer = await result.json()
      } else {
  ...
  attachEventListeners () {
    const logoutButton = this.shadow.querySelector('#logout')

    logoutButton?.addEventListener('click', async () => {
      try {
        await fetch(`${this.apiUrl}/api/auth/customer/logout`, {
          method: 'POST',
          credentials: 'include'
        })
        window.location.href = '/login-customer'
...
customElements.define('customer-panel-component', CustomerPanel)
